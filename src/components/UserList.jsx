...
function UserList({ selectID, fetchVideoList, userList, userId, idSelected }) {
  const renderList = () => {
    return userList.map((video) => {
      return (
        <SingleElement key={video.videoId + userId} videoId={video.videoId} />
      );
    });
  };
  useEffect(() => {
...
const mapStateToProps = (state) => {
  return {
    userList: Object.values(state.userList).filter(
      (el) => el.userId === state.authState.userId
    ),
